...
- **Source independence tracked by unique domain**, rather than by number of usable evidence items, so that two pages that repeat the same underlying source don't count as two separate confirmations.
